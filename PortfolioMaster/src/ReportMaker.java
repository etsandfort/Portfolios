	...
	public void printDetailedReport(ArrayList<Portfolio> portfolios){
		...
		for(int i = 0; i < portfolios.size(); i++){
			...
			if(portfolios.get(i).getBeneficiary() != null){
				nameBeneficiary = portfolios.get(i).getBeneficiary().getLastName()
								  + ", " + portfolios.get(i).getBeneficiary().getFirstName();
			}
			...
			System.out.printf("%48s %-7s %-15d %-15d %20d\n", "", "Totals:", portfolios.get(i).getTotalRisks(), portfolios.get(i).getTotalAnnualReturns(), portfolios.get(i).getTotalValue());
		}
	}
